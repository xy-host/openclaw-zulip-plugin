...
| `zulip_users` | List users, look up by ID or email, check individual or organization-wide presence status, get bot's own profile |
| `zulip_messages` | Search/fetch messages (streams, DMs, group DMs), edit/delete bot messages, add/remove emoji reactions, view edit history, forward messages with attribution |
...
| `zulip_server_settings` | Query server info, list custom profile fields, get/update user profile data |
| `zulip_message_flags` | Star/unstar messages, mark read/unread, mark all messages as read for the current account, check read receipts |
...
| `zulip_profile_fields` | Create, update, delete, or reorder custom profile fields in the organization (admin-only) |
